...
import IPy
from lib.webcore.ipAPI import IPrun
...
def iscdn(host):
    ...
    try:
        for cdn in cdns:
            if host in IPy.IP(cdn):
                result = True
    except Exception:
        pass
    return result

def check_cdn(domain, cdn_result, ip_result):
    cname = ""
    iplist = []
    try:
        ip_query = dns.resolver.query(domain,'A')
        for i in ip_query.response.answer:
            for j in i.items:
                if j.rdtype == 1:
                    iplist.append(j.address)
        ip_result[domain] = IPrun(iplist[0])
    except:
        ip_result[domain] = "查询异常"
    try:
        ...
        try:
            if len(iplist) > 1:
                cdn_result[domain] = '未知cdn'
            elif iscdn(iplist[0]):
                cdn_result[domain] = '未知cdn'
            else:
                cdn_result[domain] = 'none'
        except:
            cdn_result[domain] = '异常'
        return 1
    ...
    flag = 1 # 有cname，但不满足上述cdn标志
    for key,value in cdn_cname.items():
        if re.findall(key,cname):
            cdn_result[domain] = value
            flag = 0
            break
    if flag:
        cdn_result[domain] = "未知cdn"

def cdnrun(domainlist):
    # domainlist_deal = []
    cdn_result = {}
    ip_result = {}
    # for domain in domainlist:
    #     res = urlparse(domain)
    #     domainlist_deal.append(res.netloc)
    with ThreadPoolExecutor(max_workers=10) as executor:
        tasks = []
        for url in domainlist:
            tasks.append(executor.submit(check_cdn, url, cdn_result, ip_result))
        wait(tasks, return_when=ALL_COMPLETED)
    print("[+ IPWHERE] 归属地接口查询成功")
    print("[+ CDNDETECH] CDN接口查询成功")
    return cdn_result,ip_result


if __name__ == "__main__":
    demo = cdnrun(['https://www.tutorabc.com.cn','http://www.bilibili.com','http://apitest.chesupai.cn'])
    print(demo)
    pass
    # cname = query_cname("www.bilibili.com")
    # check_cdn(cname)
